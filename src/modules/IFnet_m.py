...
class IFNet(nn.Module):
    ...
    def forward(self, x):
        ...
        warped_img0 = flow_warp(x[:, :3], F1_large[:, :2].permute(0, 2, 3, 1))
        warped_img1 = flow_warp(x[:, 3:], F1_large[:, 2:4].permute(0, 2, 3, 1))
        flow1 = self.block1(torch.cat((warped_img0, warped_img1, F1_large), 1))
        F2 = (flow0 + flow1)
        F2_large = F.interpolate(F2, scale_factor=2.0, mode="bilinear", align_corners=False) * 2.0
        warped_img0 = flow_warp(x[:, :3], F2_large[:, :2].permute(0, 2, 3, 1))
        warped_img1 = flow_warp(x[:, 3:], F2_large[:, 2:4].permute(0, 2, 3, 1))
